class Config
{
   ...
   @{
      "GgmlDotNet.Native"     = "libGgmlDotNetNative.a";
   }
...
class ThirdPartyBuilder
{
   ...
   [string] BuildNativeGgml()
   {
      ...
      try
      {
         ...
         $NativeGgmlDir = $this._Config.GetNativeGgmlRootDir()
         $NativeGgmlTarget = Join-Path $current Ggml
         New-Item $NativeGgmlTarget -Force -ItemType Directory
         Set-Location $NativeGgmlTarget
         ...
         `"$NativeGgmlDir`"" -ForegroundColor Yellow
...
function ConfigCPU([Config]$Config, [string]$CMakefileDir)
{
   ...
   # Build Ggml
   $installNativeGgmlDir = $Builder.BuildNativeGgml()
   ...
   if ($IsWindows)
   {
      ...
      $env:ggml_DIR = $installNativeGgmlDir
      Write-Host "   cmake -G `"${VS}`" -A ${VSARC} -T host=x64 `
         -D ggml_DIR=`"${installNativeGgmlDir}`" `
...
function ConfigUWP([Config]$Config, [string]$CMakefileDir)
{
   if ($IsWindows)
   {
      ...
      $NativeGgmlDir = $Config.GetNativeGgmlRootDir()
      $patchFullPath = Join-Path $nugetDir $patch
      $current = Get-Location
      Set-Location -Path $NativeGgmlDir
      Write-Host "Apply ${patch} to ${NativeGgmlDir}" -ForegroundColor Yellow
...
function Reset-Ggml-Modification([Config]$Config, [string]$currentDir)
{
   $NativeGgmlDir = $Config.GetNativeGgmlRootDir()
   Set-Location -Path $NativeGgmlDir
   Write-Host "Reset modification of ${NativeGgmlDir}" -ForegroundColor Yellow
   git checkout .
...
function Build([Config]$Config)
{
   ...
   # revert Ggml
   Reset-Ggml-Modification $Config (Join-Path $Current $Output)
