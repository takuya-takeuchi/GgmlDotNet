...
$ggmldotnetRoot = Split-Path $ScriptPath -Parent
$BuildUtilsPath = Join-Path $ggmldotnetRoot "nuget" | `
                  Join-Path -ChildPath "BuildUtils.ps1"
import-module $BuildUtilsPath -function *
$Config = [Config]::new($ggmldotnetRoot, "Release", $Target, $Architecture, $Platform, $Option)

if ($Target -ne "cuda")
{
   $postfix = $Option
   $cuda = 0
   $dockername = "ggmldotnet/test/$Distribution/$DistributionVersion/$Target" + $postfix
}
else
{
   $cudaVersion = ($Option / 10).ToString("0.0")
   $cuda = 1
   $dockername = "ggmldotnet/test/$Distribution/$DistributionVersion/$Target/$cudaVersion"
}
...
if ($cuda -ne 0)
{
   ...
   if ($dockerAPIVersion -ge 1.40)
   {
      docker run --gpus all --rm `
                 --entrypoint="/bin/bash" `
                 -v "$($ggmldotnetRoot):/opt/data/ggmldotnet" `
                 -e "LOCAL_UID=$(id -u $env:USER)" `
                 -e "LOCAL_GID=$(id -g $env:USER)" `
                 -w "/opt/data/ggmldotnet" `
                 -it "$dockername"
   }
   else
   {
